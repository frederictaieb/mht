...
const OUTPUT_URL = (name: string, version?: number) =>
  `${API_BASE}/cinemai/static/output_video/${encodeURIComponent(name)}${version ? `?v=${version}` : ""}`
...
export default function FaceswapVideoCard({ isReady, img, vid }: Props) {
  ...
  const outputUrl = useMemo(() => {
    ...
    return out ? OUTPUT_URL(out, Date.now()) : null
  ...
  const handleGenerateFaceswapVideo = async () => {
    ...
    try {
      ...
      console.error("Faceswap generation error:", err)
